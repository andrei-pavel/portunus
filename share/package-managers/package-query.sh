...
function purge_ {
  local package="${1}"
  package-query -Rsc --noconfirm "${package}"
  return "${?}"
}
...
function autoremove_ {
  to_be_removed=$(package-query -Qtd --aur --noconfirm 2>&1 | cut -d ' ' -f 1 | cut -d '/' -f 2 | head -n -3)
  if [[ ! -z ${to_be_removed} ]]; then
    echo "${to_be_removed}" | xargs package-query -Rsc --noconfirm
  fi
  return "${?}"
}
