...
function install_ {
  local package="${1}"
  aurutils -Syy --aur --overwrite '*' --needed --noconfirm "${package}"
  return "${?}"
}
...
function update_ {
  aurutils -U --aur --overwrite '*' --needed --noconfirm
  return "${?}"
}

function upgrade_ {
  if [[ ${#} -eq 0 ]]; then
    aurutils -Su --aur --overwrite '*' --needed --noconfirm
    return "${?}"
  fi

  local package="${1}"
  aurutils -Su --overwrite '*' --needed --noconfirm "${package}"
  return "${?}"
}
